...
public class CastCreeperBookItem extends Item {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(Component.translatable("tooltip.youareherobrineresurrected.cast_creeper_book.tooltip"));
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!pLevel.isClientSide) {
            ...
            serverLevel.sendParticles(
                    ParticleTypes.FLAME,
                    pPlayer.getX(), pPlayer.getY() + 1, pPlayer.getZ(),
                    40,
                    0.35D, 0.5D, 0.35D,
                    0.02D
            );
            ...
            for (BlockPos pos : spawnPositions) {
                Creeper mob = EntityType.CREEPER.create(serverLevel);
